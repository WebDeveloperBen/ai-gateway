...
import { VueDataUi, type VueUiXyDatasetItem, type VueUiXyConfig } from "vue-data-ui"
...
function asDataset(values: number[], name: string, type: ChartType = "line", dataLabels = false): VueUiXyDatasetItem[] {
...
function xyConfig(
  ...
  return {
    ...
    customPalette: ["var(--chart-1)", "var(--chart-2)", "var(--chart-3)", "var(--chart-4)", "var(--chart-5)"],
    chart: {
...
const dataset = computed(() => asDataset(props.series, props.seriesName ?? "Series", props.type ?? "line"))
...
<template>
  <VueDataUi component="VueUiXy" :dataset="dataset" :config="config" class="w-full h-full" />
</template>
